...
import { Play, Plus, Sparkles, Calendar, Eye, Star, Trash2 } from 'lucide-react';
...
export default function AnimationView({
  ...
  // Gather unique tags across all animations
  const allTags = ['All', ...Array.from(new Set(animations.flatMap((a) => a.tags)))];
  ...
  return (
    <div id="animations-view" className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-black uppercase tracking-widest text-neutral-500 mt-1">
            Immersive 3D renderings, CGI breakthroughs, and stylized cartoon series.
          </p>
        </div>

        {/* Add Animation Action Button */}
        <button
          id="btn-add-animation-view"
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
        {allTags.map((tag) => (
          ...
          >
            {tag === 'All' ? 'All Animations' : tag}
          </button>
        ))}
      </div>

      {/* Animations Cards Grid */}
      {filteredAnimations.length === 0 ? (
        <div className="text-center py-20 bg-neutral-900 border border-neutral-800 rounded-none">
          <Sparkles size={48} className="mx-auto text-neutral-600 mb-4 stroke-[1.5]" />
          <p className="text-neutral-400 font-sans text-sm font-bold">No animations found matching your filter.</p>
          <p className="text-xs text-neutral-500 mt-2 font-medium">Try choosing a different tag or uploading an MP4 file.</p>
        </div>
      ) : (
          ...
          {filteredAnimations.map((animation) => (
            ...
            >
              ...
              >
                ...
                <span className="absolute bottom-3 left-3 bg-neutral-950/80 border border-neutral-800 text-[9px] text-neutral-300 px-2.5 py-1 rounded-none font-black tracking-widest font-mono">
                  {animation.duration}
                </span>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div className="space-y-2" onClick={() => onPlayVideo(animation)}>
                ...
                <div className="flex items-center justify-between pt-4 mt-4 border-t border-neutral-800">
                  <div className="flex items-center gap-3 text-[9px] text-neutral-500 font-black uppercase tracking-widest">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {animation.views || '0'} views
                    </span>
                  </div>

                  <div className="flex items-center gap-2">
                    {onDeleteVideo && !animation.id.startsWith('v') && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onDeleteVideo(animation.id);
                        }}
                        className="p-1.5 rounded-none text-neutral-500 hover:text-red-400 hover:bg-neutral-800 transition-all"
                        title="Delete custom animation"
                      >
                        <Trash2 size={13} />
                      </button>
                    )}
